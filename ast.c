        ...
        case TOK_OPERATOR:
          {
            ...
            if (strstr ("+-*/%", (char *)(char[]){ *op, '\0' })
                && op[1] == '\0')
              {
                ...
                if (e.type != -1)
                  {
                    em = SFMALLOC (sizeof (*em));
                    *em = e;
                    nodes[nl++] = (arith_node_t){ .type = ARITH_NODE_E_O,
                                                  .v.expr = em };
                  }
                    ...
                    if (t.type == TOK_OPERATOR)
                      {
                        ...
                        if (strstr ("+-*/%", (char *)(char[]){ *op, '\0' })
                            && op[1] == '\0' && !gb)
                          {
                            ...
                            if (nl > 0
                                && nodes[nl - 1].v.expr->type != EXPR_CONST)
                              all_consts = 0;
                ...
                /* +c or -c */
                else if (nl == 2 && nodes[0].type == ARITH_NODE_OPERATOR
                         && (!strcmp (nodes[0].v.op, "-")
                             || !strcmp (nodes[0].v.op, "+")))
                  {
                    nodes[2] = nodes[1];
                    ...
                    expr_t *e_zr = SFMALLOC (sizeof (*e_zr));
                    *e_zr = (expr_t){
                      .type = EXPR_CONST,
                      .v.e_const.v.type = CONST_INT,
                      .v.e_const.v.v.c_int.v = 0,
                    };

                    nodes[0] = (arith_node_t){ .type = ARITH_NODE_E_O,
                                               .v.expr = e_zr };
                    nl++;
            ...
            else if (*op == '{')
              {
                ...
                expr_t **keys = SFMALLOC (64 * sizeof (*keys));
                expr_t **vals = SFMALLOC (64 * sizeof (*vals));
                size_t l = 0;
                    ...
                    if (u.type == TOK_OPERATOR)
                      {
                        ...
                        if (*op == '}' && !gb)
                          {
                            if (left == start - 1)
                              ...
                              {
                                vals[l++] = sf_expr_gen (left, start - 1);
                                left = start;
                                continue;
                              }
                        ...
                        if (*op == ':' && !gb)
                          {
                            saw_colon = 1;
                            keys[l] = sf_expr_gen (left, start - 1);
                            left = start;
                            continue;
                          }
                ...
                e.type = EXPR_DICT;
                e.v.e_dict.keys = keys;
                e.v.e_dict.vals = vals;
                e.v.e_dict.l = l;
                goto end;
